...
Pay(bill: Bill) {
  ...
  /*
      // Move the bill from UnPaid to Paid section in Firebase Realtime Database
      this.http.get<Bill>(unpaidUrl).subscribe((unpaidBills) => {
       
        
          // Remove the bill from UnPaid
          
         this.http.delete<Bill>(deleteUrl).subscribe(()=> {
          
          
          update(ref(db, `users/${dn}/Bills/ElecBill`), {
            
            Paid: [...(this.bills1 ? this.bills1 : []), { ...bill, paidDate: new Date().toISOString() }]
          }).then(() => {
            // Update the user's balance in Firebase Realtime Database
            update(ref(db, `users/${dn}/Wallet`), {
              Balance: newbalance
            }).then(() => {
              // Update the balance in the component
              this.balance = newbalance;
              // Show a success message
              this.toastr.success(`Payment successful! You paid ${bill.price} for ${bill.units} units.`);
            }).catch((error) => {
              console.log('Error updating balance:', error);
              this.toastr.error('An error occurred while updating your balance. Please try again later.', undefined, toastConfig);
            });
          }).catch((error) => {
            console.log('Error moving bill to Paid:', error);
            this.toastr.error('An error occurred while moving the bill to the Paid section. Please try again later.', undefined, toastConfig);
          });
        });
         
          console.log('No unpaid bills found.');
          this.toastr.warning('No unpaid bills found.');
        
      }, (error) => {
        console.log('Error getting unpaid bills:', error);
        this.toastr.error('An error occurred while getting your unpaid bills. Please try again later.', undefined, toastConfig);
      });
    } else {
      console.log('Insufficient balance.');
      this.toastr.warning('You have insufficient balance to pay this bill. Please recharge your wallet.', undefined, toastConfig);
